...
static void printOpcode(struct disassemblyContext *vctx, const char *aOpcode, uint16_t aFlags) {
	vctx->printDisassembly(vctx, "%s", aOpcode);
	if ((aFlags & OPCODE_SETFLAGS) || (((vctx->itState & 0x03) == IT_NONE) && (aFlags & OPCODE_SETFLAGS_IF_UNCOND))) {
		vctx->printDisassembly(vctx, "s");
	}
	if ((aFlags & OPCODE_FORCE_CONDITION) == 0) {
		/* Honor implicit condition flags */
		if ((vctx->itState & 0x03) == IT_THEN) {
			vctx->printDisassembly(vctx, "%s", ccString(vctx->itCond));
		} else if ((vctx->itState & 0x03) == IT_ELSE) {
			vctx->printDisassembly(vctx, "%s", ccString(vctx->itCond ^ 1));
		}
	} else {
		/* Override with given condition flag */
		vctx->printDisassembly(vctx, "%s", ccString((aFlags & 0xf00) >> 8));
	}
	if (aFlags & OPCODE_WIDE) {
		vctx->printDisassembly(vctx, ".w");
	} else if (aFlags & OPCODE_NARROW) {
		vctx->printDisassembly(vctx, ".n");
	}
}

static void print_reglist(struct disassemblyContext *vctx, uint16_t aRegList) {
	bool first = true;
	vctx->printDisassembly(vctx, " {");
	for (int i = 0; i < 16; i++) {
		if (aRegList & (1 << i)) {
			if (!first) {
				vctx->printDisassembly(vctx, ", ");
			...
			vctx->printDisassembly(vctx, "%s", regString(i));
		}
	}
	vctx->printDisassembly(vctx, "}");
}

static void print_i(struct disassemblyContext *vctx, uint8_t aImmediate) {
	vctx->printDisassembly(vctx, " #%d", aImmediate);
}

static void print_r(struct disassemblyContext *vctx, uint8_t aR1) {
	vctx->printDisassembly(vctx, " %s", regString(aR1));
}

static void print_rr(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2) {
	vctx->printDisassembly(vctx, " %s, %s", regString(aR1), regString(aR2));
}

static void print_rrr(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aR3) {
	vctx->printDisassembly(vctx, " %s, %s, %s", regString(aR1), regString(aR2), regString(aR3));
}

static void print_rrrr(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aR3, uint8_t aR4) {
	vctx->printDisassembly(vctx, " %s, %s, %s, %s", regString(aR1), regString(aR2), regString(aR3), regString(aR4));
}

static void print_s(struct disassemblyContext *vctx, uint8_t aType, uint8_t aImm) {
	if (aType == TYPE_LSL) {
		vctx->printDisassembly(vctx, ", lsl #%d", aImm);
	} else if (aType == TYPE_LSR) {
		if (aImm == 0) {
			aImm = 32;
		}
		vctx->printDisassembly(vctx, ", lsr #%d", aImm);
	} else if (aType == TYPE_ASR) {
		if (aImm == 0) {
			aImm = 32;
		}
		vctx->printDisassembly(vctx, ", asr #%d", aImm);
	} else {
		if (aImm == 0) {
			vctx->printDisassembly(vctx, ", rrx");
		} else {
			vctx->printDisassembly(vctx, ", ror #%d", aImm);
		}
	}
}

static void print_Irr(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2) {
	vctx->printDisassembly(vctx, " [%s, %s]", regString(aR1), regString(aR2));
}

static void print_Irrs(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aType, uint8_t aImm) {
	vctx->printDisassembly(vctx, " [%s, %s", regString(aR1), regString(aR2));
	print_s(vctx, aType, aImm);
	vctx->printDisassembly(vctx, "]");
}

static void print_rrs(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aType, uint8_t aImm) {
	vctx->printDisassembly(vctx, " %s, %s", regString(aR1), regString(aR2));
	if (aType || aImm) {
		print_s(vctx, aType, aImm);
	}
...
static void print_rIrrs(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aR3, uint8_t aType, uint8_t aImm) {
	vctx->printDisassembly(vctx, " %s, [%s, %s", regString(aR1), regString(aR2), regString(aR3));
	if (aType || aImm) {
		print_s(vctx, aType, aImm);
	}
...
static void print_rrrs(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aR3, uint8_t aType, uint8_t aImm) {
	vctx->printDisassembly(vctx, " %s, %s, %s", regString(aR1), regString(aR2), regString(aR3));
	if (aType || aImm) {
		print_s(vctx, aType, aImm);
	}
...
static void print_rri(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, int aImmediate) {
	vctx->printDisassembly(vctx, " %s, %s, #%u", regString(aR1), regString(aR2), aImmediate);
}

static void print_rrii(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, int aImmediate1, int aImmediate2) {
	vctx->printDisassembly(vctx, " %s, %s, #%u, #%u", regString(aR1), regString(aR2), aImmediate1, aImmediate2);
}

static void print_rIro(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, int aImmediate) {
	vctx->printDisassembly(vctx, " %s, [%s, #%d]", regString(aR1), regString(aR2), aImmediate);
}

static void print_rrIrO(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aR3, int aImmediate) {
	if (aImmediate) {
		vctx->printDisassembly(vctx, " %s, %s, [%s, #%d]", regString(aR1), regString(aR2), regString(aR3), aImmediate);
	} else {
		vctx->printDisassembly(vctx, " %s, %s, [%s]", regString(aR1), regString(aR2), regString(aR3));
	}
}
...
static void print_rIri(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, int aImmediate) {
	vctx->printDisassembly(vctx, " %s, [%s], #%d", regString(aR1), regString(aR2), aImmediate);
}

static void print_rrIri(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aR3, int aImmediate) {
	vctx->printDisassembly(vctx, " %s, %s, [%s], #%d", regString(aR1), regString(aR2), regString(aR3), aImmediate);
}

static void print_rIrr(struct disassemblyContext *vctx, uint8_t aR1, uint8_t aR2, uint8_t aR3) {
	vctx->printDisassembly(vctx, " %s, [%s, %s]", regString(aR1), regString(aR2), regString(aR3));
}

static void print_ri(struct disassemblyContext *vctx, uint8_t aR1, int aImmediate) {
	vctx->printDisassembly(vctx, " %s, #%u", regString(aR1), aImmediate);
}

static void disassembly_i32_adc_imm_T1(void *vctx, uint8_t Rd, uint8_t Rn, int32_t imm, bool S) {
	struct disassemblyContext *ctx = (struct disassemblyContext*)vctx;
